...
INSERT INTO orm_abc_app_abcmodel ('task', 'a', 'b','c', 'int','result') VALUES('Находится ли С в диапазоне A B?', 1, 2, 3, 1,  'не известно');
SELECT * FROM  orm_abc_app_abcmodel;
...
SELECT id, task AS Формулировка, a, b, c, int,  CASE WHEN c > a AND c < b THEN 'с в диапазоне a-b' WHEN c > b AND c < a THEN 'с в диапазоне a-b' ELSE 'с вне диапазона a-b' END AS Результат FROM orm_abc_app_abcmodel;
